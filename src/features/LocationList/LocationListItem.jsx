...
function LocationListItem({ location, editLocation, deleteLocation }) {
  ...
  /*Fetches current weather information for saved location*/
  useEffect(() => {
  ...
  /*Clicking the location's title sets this location as the current location and navigates to the Home Page*/
  ...
  return (
    <tr key={workingLocation.id}>
      ...
      <td>{workingLocation.latitude}</td>
      <td>{workingLocation.longitude}</td>
      <td>{currentTemp}°F</td>
      <td>{translateCode(currentWeatherCode)}</td>
